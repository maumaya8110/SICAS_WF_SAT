...
ORDER BY	E.Nombre, U.NumeroEconomico";
    ...
    public class Vista_Reporte_FlujoCaja
    {
        ...
        public static List<Vista_Reporte_FlujoCaja> Get(
            System.Int32? sesion_id)
        {
            string sqlstr = @"SELECT	CFC.CuentaFlujoCaja_ID,
		CFC.Sesion_ID,
		C.Empresa_ID,
		E.Nombre Empresa,
		C.Estacion_ID,
		...
		CFC.Cargo,
		CFC.Abono,
		(SELECT SUM(Abono - Cargo) 
		FROM CuentaFlujoCajas
		WHERE	Sesion_ID = S.Sesion_ID
		AND	Fecha <= CFC.Fecha) Saldo,
		CFC.Fecha,
		CFC.Usuario_ID
FROM	CuentaFlujoCajas CFC
INNER JOIN	Sesiones S
ON		S.Sesion_ID = CFC.Sesion_ID
INNER JOIN	Cajas C
ON		C.Caja_ID = CFC.Caja_ID
INNER JOIN	Monedas M
ON		M.Moneda_ID = CFC.Moneda_ID
LEFT JOIN	Tickets T
ON		T.Ticket_ID = CFC.Ticket_ID
INNER JOIN	Empresas E
ON		C.Empresa_ID = E.Empresa_ID
INNER JOIN	Estaciones EST
ON		C.Estacion_ID = EST.Estacion_ID
WHERE	( @Sesion_ID IS NULL OR CFC.Sesion_ID = @Sesion_ID )
ORDER BY	CFC.Fecha";
...
ORDER BY	E.Nombre, EST.Nombre, U.NumeroEconomico";
    ...
    public class Vista_LicenciasPorVencer
    {
            ...
            System.Int32 estacion_id)
        {
            ...
            if (estacion_id != 5)
            {
                sqlstr = @"SELECT	DISTINCT COND.Conductor_ID,
		                            UPPER(COND.Apellidos + ' ' + COND.Nombre) Nombre,
		                            COND.FolioLicencia LicenciaLiberada,
		                            CONVERT(DATE, COND.VenceLicencia) VenceLicencia
                            FROM	Conductores COND
		                            LEFT JOIN Contratos CONT ON CONT.Conductor_ID = COND.Conductor_ID
                            WHERE	COND.VenceLicencia IS NOT NULL
                            AND		COND.VenceLicencia <= DATEADD(MONTH,6,GETDATE())
                            AND		( @Estacion_ID IS NULL OR COND.Estacion_ID = @Estacion_ID )
    ...
    public class Vista_ConductoresOperativos
    {
        ...
        public static List<Vista_ConductoresOperativos> Get(
            System.Int32? empresa_id,
             ...
             System.Int32? numeroeconomico)
        {
            string sqlstr = @"SELECT	C.Conductor_ID,
...
ORDER BY	Nombre";
    ...
    public class Vista_Usuarios_Estaciones
    {
        ...
        public static List<Vista_Usuarios_Estaciones> Get(
            System.String usuario_id)
        {
            string sqlstr = @"SELECT	Estacion_ID, Nombre,
		(	SELECT	COUNT(*)
    ...
    public class Vista_Conceptos
    {
        ...
        public static List<Vista_Conceptos> Get(
            System.Int32? concepto_id,
             System.Int32? cuenta_id)
        {
            string sqlstr = @"SELECT	C.Concepto_ID,
		C.Nombre,
		C.Cuenta_ID,
		CU.Nombre Cuenta,
		C.EnRecibo,
...
AND		( @Cuenta_ID IS NULL OR C.Cuenta_ID = @Cuenta_ID )";
    ...
    public class Vista_Empresas_Cuentas
    {
        ...
        public static List<Vista_Empresas_Cuentas> Get(
            System.Int32? empresa_id)
        {
            string sqlstr = @"SELECT	EC.Empresa_ID,
		E.Nombre Empresa,
		EC.Cuenta_ID,		
		C.Nombre Cuenta
...
WHERE	( @Usuario_ID IS NULL OR U.Usuario_ID = @Usuario_ID )";
    ...
    public class Vista_ServiciosRegresos
    {
		...
		C.Nombre + ' ' + C.Apellidos Conductor,
		U.NumeroEconomico Unidad,
		S.Precio,
		S.PorcentajeRegreso,
		S.ComisionRegreso,
    ...
    public class Vista_MatrizVentasTaller
    {
        ...
        public static List<Vista_MatrizVentasTaller> Get(
            System.Object fechainicial,
             ...
             System.Object estacion_id)
        {
            string sqlstr = @"SELECT	dbo.udf_DateValue(OT.FechaTerminacion) Fecha,
		E.Nombre Cliente,
        F.Nombre Familia, 
        SUM(OS.Total) Total,
        'MANO OBRA' Concepto
...
GROUP BY	dbo.udf_DateValue(OT.FechaTerminacion), E.Nombre, F.Nombre

UNION

SELECT	dbo.udf_DateValue(OT.FechaTerminacion) Fecha,
		E.Nombre Cliente,
...
AND		OT.Empresa_ID = @Empresa_ID
AND		OT.Estacion_ID = @Estacion_ID
GROUP BY	dbo.udf_DateValue(OT.FechaTerminacion), E.Nombre, CASE OT.TipoMantenimiento_ID	
...
FROM	EstatusValesPrepagados";
    ...
    public class Vista_CortesDeVentas
    {
		...
		S.FechaInicial, 
		S.FechaFinal, 
		dbo.udf_DateValue(S.FechaInicial) FechaCorte
FROM	Sesiones S
INNER JOIN	Estaciones EST
...
AND		( 
			...
			( dbo.udf_DateValue(S.FechaInicial) BETWEEN @FechaInicial AND @FechaFinal ) 
		)
AND		S.Sesion_ID IN
(
...
ON		SM.ServicioMantenimiento_ID = SMP.ServicioMantenimiento_ID
    ...
    public class Vista_RegistroPublicitario
    {
		...
		UPPER(C.Nombre + ' ' + C.Apellidos) Nombre, C.Telefono, 
...
FROM	CuentaConductores CC
	...
	INNER JOIN	Conceptos CON
		ON		CC.Concepto_ID = CON.Concepto_ID		
WHERE	 dbo.udf_DateValue(CC.Fecha) = dbo.udf_DateValue(@Fecha)
...
WHERE	dbo.udf_DateValue(SES.FechaInicial) = dbo.udf_DateValue(@Fecha)";
...
AND		OT.Empresa_ID = @Empresa_ID
AND		OT.Estacion_ID = @Estacion_ID
ORDER BY dbo.udf_DateValue(OT.FechaTerminacion), 
       E.Nombre,
       M.Nombre,
       TM.Nombre,
       OT.Total";
    ...
    public class Vista_ComprasConsolidadas
    {
		...
		dbo.udf_DateValue(OC.Fecha) Fecha,
...
AND		( @Estacion_ID IS NULL OR PR.Estacion_ID = @Estacion_ID )";
    ...
    public class Vista_ContratosLiquidados
    {
        ...
        public Vista_ContratosLiquidados(System.Int32 folio, System.String conductor, System.Int32 unidad, System.String locacionunidad, System.String estatusconductor, System.String estatuscontrato, System.DateTime fechaliquidacion, System.String usuarioliquidacion, System.String comentario)
        {
            this.Folio = folio;
            this.Conductor = conductor;
            this.Unidad = unidad;
        ...
        public static List<Vista_ContratosLiquidados> Get(
            ...
            System.DateTime? fechafinal)
        {
            string sqlstr = "SELECT	CL.Contrato_ID Folio, COND.Nombre + ' ' +  COND.Apellidos Conductor, \r\n" +
            "		U.NumeroEconomico Unidad, LU.Nombre LocacionUnidad, \r\n" +
            "		EC.Nombre EstatusConductor, \r\n" +
            "		ECO.Nombre EstatusContrato, \r\n" +
            "		CL.Fecha FechaLiquidacion, \r\n" +
            "		CL.Usuario_ID UsuarioLiquidacion, \r\n" +
            "		CL.Comentarios Comentario \r\n" +
            "FROM	ContratosLiquidados CL \r\n" +
            "INNER JOIN	Contratos C \r\n" +
            "ON		CL.Contrato_ID = C.Contrato_ID \r\n" +
            "INNER JOIN	Conductores COND \r\n" +
            "ON		COND.Conductor_ID = CL.Conductor_ID \r\n" +
            "INNER JOIN	Unidades U \r\n" +
            "ON		CL.Unidad_ID = U.Unidad_ID \r\n" +
            "INNER JOIN	EstatusConductores EC \r\n" +
            "ON		EC.EstatusConductor_ID = CL.EstatusConductor_ID \r\n" +
            "INNER JOIN	EstatusContratos ECO \r\n" +
            "ON		ECO.EstatusContrato_ID = CL.EstatusContrato_ID \r\n" +
            "INNER JOIN	LocacionesUnidades LU \r\n" +
            "ON		CL.LocacionUnidad_ID = LU.LocacionUnidad_ID \r\n" +
            "WHERE	( @Empresa_ID IS NULL OR C.Empresa_ID = @Empresa_ID ) \r\n" +
            "AND		( @Estacion_ID IS NULL OR C.Estacion_ID = @Estacion_ID ) \r\n" +
            "AND		( (@FechaInicial IS NULL OR @FechaFinal IS NULL)  \r\n" +
            "			OR CL.Fecha BETWEEN @FechaInicial AND @FechaFinal)";
    ...
    public class Vista_ReporteLicenciasVencidas
    {
        ...
        public static List<Vista_ReporteLicenciasVencidas> Get()
        {
            ...
            "ON		( C.Conductor_ID = CONT.Conductor_ID  \r\n" +
            "			AND CONT.EstatusContrato_ID = 1  \r\n" +
            "			AND CONT.Unidad_ID IS NOT NULL ) \r\n" +
...
ORDER BY EST.Nombre, C.NumeroEconomico, CO.Apellidos, CO.Nombre;";
    ...
    public class Vista_ReporteVentasTaller
    {
		...
		dbo.udf_DateValue(OT.FechaTerminacion) Fecha,
		E.Nombre Empresa,
...
ORDER BY	U.NumeroEconomico, COND.Apellidos + ' ' + COND.Nombre";
    ...
    public class Vista_SaldosConductores
    {
        ...
        {
            string sqlstr = @"SELECT	Cont.Unidad_ID, U.NumeroEconomico Unidad,
        CC.Conductor_ID, (ISNULL(CONVERT(varchar(10),U.NumeroEconomico) + '-','') + C.Apellidos + ' ' + C.Nombre) Conductor,
        CC.Empresa_ID, E.Nombre Empresa,
        CC.Cuenta_ID, Cu.Nombre Cuenta,
        CC.Saldo
FROM	
(
SELECT	Empresa_ID, Cuenta_ID, Conductor_ID, (SUM(Abono) - SUM(Cargo)) Saldo
FROM	CuentaConductores
GROUP BY	Empresa_ID, Cuenta_ID, Conductor_ID

)	CC
INNER JOIN	Conductores C
ON		CC.Conductor_ID = C.Conductor_ID
INNER JOIN	Empresas E
ON		CC.Empresa_ID = E.Empresa_ID
INNER JOIN	Cuentas Cu
ON		CC.Cuenta_ID = Cu.Cuenta_ID
LEFT JOIN	Contratos Cont
ON		(C.Conductor_ID = Cont.Conductor_ID AND Cont.EstatusContrato_ID = 1)
LEFT JOIN	Unidades U
ON		Cont.Unidad_ID = U.Unidad_ID
WHERE	C.Conductor_ID = @Conductor_ID";
        ...
        {
            string tipojoin = "LEFT";
            if (activo)
            {
                tipojoin = "INNER";
            }
            string sqlstr1 = @"SELECT	Cont.Unidad_ID, U.NumeroEconomico Unidad,
        CC.Conductor_ID, (ISNULL(CONVERT(varchar(10),U.NumeroEconomico) + '-','') + C.Apellidos + ' ' + C.Nombre) Conductor,
        CC.Empresa_ID, E.Nombre Empresa,
        CC.Cuenta_ID, Cu.Nombre Cuenta,
        CC.Saldo
...
ON		CC.Cuenta_ID = Cu.Cuenta_ID
";
string sqlstr2 = @" JOIN	Contratos Cont
ON		(C.Conductor_ID = Cont.Conductor_ID AND Cont.EstatusContrato_ID = 1)
LEFT JOIN	Unidades U
ON		Cont.Unidad_ID = U.Unidad_ID
INNER JOIN	Usuarios_Empresas UE
ON		( UE.Empresa_ID = CC.Empresa_ID
...
AND		( @NumeroEconomico IS NULL OR U.NumeroEconomico = @NumeroEconomico )
AND		( @Nombre IS NULL OR (C.Apellidos + ' ' + C.Nombre) LIKE @Nombre + '%' )";
            string sqlstr = sqlstr1 + tipojoin + sqlstr2;
            Hashtable m_params = new Hashtable();
            m_params.Add("@Empresa_ID", empresa_id);
            m_params.Add("@Estacion_ID", estacion_id);
            m_params.Add("@NumeroEconomico", numeroeconomico);
...
SELECT	E.Nombre Empresa, 
		...
		Cont.MontoDiario, 
		Cont.FechaInicial,
		(SELECT dbo.udf_DateValue(ISNULL(MAX(Fecha),GETDATE())) FROM CuentaConductores
		...
		ELSE 0
		END DiasSinPagar,
		DATEDIFF(dd, (SELECT dbo.udf_DateValue(ISNULL(MAX(Fecha),GETDATE())) FROM CuentaConductores
		WHERE	Conductor_ID = Cont.Conductor_ID
		AND		Concepto_ID = 2), dbo.udf_DateValue(GETDATE())) DiasSinPresentarse,
...
WHERE	( Cont.Empresa_ID IN ( SELECT Empresa_ID FROM Usuarios_Empresas WHERE Usuario_ID = @Usuario_ID ) )
AND		( Cont.Estacion_ID IN ( SELECT Estacion_ID FROM Usuarios_Estaciones WHERE Usuario_ID = @Usuario_ID ) )
AND     Cont.EstatusContrato_ID = 1
...
BEGIN	
	...
	BEGIN
		...
		SELECT	U.Unidad_ID, U.NumeroEconomico, U.Placas, 
				E.Nombre Empresa, U.Empresa_ID, 
				CO.Conductor_ID, 
				(CO.Nombre + ' ' + CO.Apellidos) Conductor,
				MU.ModeloUnidad_ID,
...
END";
    ...
    public class SaldosCuentaCajasPorFechas
    {
        ...
        {
            string sqlstr = "SELECT	dbo.udf_DateValue(CC.Fecha) Fecha, E.Nombre Empresa, CU.Nombre Cuenta,  \r\n" +
            "		SUM(Abono - Cargo) Saldo \r\n" +
            "FROM	CuentaCajas CC \r\n" +
            "INNER JOIN	Cajas C \r\n" +
            ...
            "GROUP BY	dbo.udf_DateValue(CC.Fecha), E.Nombre, CU.Nombre \r\n" +
            "ORDER BY	dbo.udf_DateValue(CC.Fecha), E.Nombre, CU.Nombre";
...
ORDER BY	Conductor DESC";
    ...
    public class PrecioServicio
    {
        ...
        public static PrecioServicio Get(System.Int32 zona_id, System.Int32? tiposervicio_id)
        {
            string sqlstr = "SELECT	dbo.Tarifas.Tarifa Precio \r\n" +
            "FROM	dbo.Tarifas \r\n" +
            "WHERE	dbo.Tarifas.Zona_ID = @Zona_ID \r\n" +
            "AND		dbo.Tarifas.TipoServicio_ID = @TipoServicio_ID";
        ...
        public static DataTable GetDataTable(System.Int32 zona_id, System.Int32? tiposervicio_id)
        {
            string sqlstr = "SELECT	dbo.Tarifas.Tarifa Precio \r\n" +
            "FROM	dbo.Tarifas \r\n" +
            "WHERE	dbo.Tarifas.Zona_ID = @Zona_ID \r\n" +
    ...
    public class BoletoVenta
    {
        ...
        public static BoletoVenta Get(System.String servicio_id)
        {
            ...
            LEFT JOIN	Unidades U    
            ON		S.Unidad_ID = U.Unidad_ID    
            LEFT JOIN	ClasesServicios CS    
            ON		S.ClaseServicio_ID = CS.ClaseServicio_ID
            LEFT JOIN	Conductores C
            ON		S.Conductor_ID = C.Conductor_ID 
            CROSS JOIN	VariablesNegocio VN    
    ...
    public class Vista_ComisionesServicios
    {
        ...
        {
            string sqlstr = "SELECT	CS.ComisionServicio_ID, CS.Nombre, CS.Comisionista_ID, E.Nombre Comisionista, \r\n" +
            "		CS.TipoComision_ID, TC.Nombre TipoComision, CS.Monto \r\n" +
            "FROM	ComisionesServicios CS \r\n" +
            "INNER JOIN	TiposComisiones TC \r\n" +
            "ON		CS.TipoComision_ID = TC.TipoComision_ID \r\n" +
            "INNER JOIN	Empresas E \r\n" +
            "ON		CS.Comisionista_ID = E.Empresa_ID \r\n" +
            "WHERE	( @ComisionServicio_ID IS NULL OR CS.ComisionServicio_ID = @ComisionServicio_ID ) \r\n" +
            "AND		( @Comisionista_ID IS NULL OR CS.Comisionista_ID = @Comisionista_ID ) \r\n" +
            "AND		( @TipoComision_ID IS NULL OR TC.TipoComision_ID = @TipoComision_ID ) \r\n" +
            "AND		( @Nombre IS NULL OR CS.Nombre LIKE @Nombre + '%' ) \r\n" +
            "AND		( @Monto IS NULL OR CS.Monto = @Monto )";
    ...
    public class Vista_VentasServiciosDeSesion_Consolidado
    {
        ...
        public static List<Vista_VentasServiciosDeSesion_Consolidado> Get(
            System.Object sesion_id)
        {
            string sqlstr = @"SELECT	Z.Nombre Zona, S.Precio, COUNT(S.Servicio_ID) Cantidad, SUM(Precio) Total
FROM	Servicios S
INNER JOIN	Zonas Z
ON		S.Zona_ID = Z.Zona_ID
WHERE	S.Sesion_ID = @Sesion_ID
AND		TipoServicio_ID IS NOT NULL
AND		TipoServicioConductor_ID IS NULL
AND		S.EstatusServicio_ID <> 4
GROUP BY	Z.Nombre, S.Precio";
...
GROUP BY	Z.Nombre, S.Precio";
        ...
        {
            string sqlstr = @"SELECT	ISNULL(Z.Nombre,TSC.Nombre) Zona,
        S.Precio,
		COUNT(S.Servicio_ID) Cantidad, 
		SUM(S.Precio) Total
FROM	Servicios S
LEFT JOIN	Zonas Z
ON		S.Zona_ID = Z.Zona_ID
...
WHERE	dbo.udf_DateValue(SES.FechaInicial) = dbo.udf_DateValue(@Fecha)
AND		S.EstatusServicio_ID <> 4
GROUP BY	ISNULL(Z.Nombre,TSC.Nombre), S.Precio";
    ...
    public class Vista_ServiciosConductorDeSesion_Consolidado
    {
        ...
        public static List<Vista_ServiciosConductorDeSesion_Consolidado> Get(
            System.Object sesion_id)
        {
            string sqlstr = @"SELECT	TSC.Nombre Zona, S.Precio, COUNT(S.Servicio_ID) Cantidad, SUM(S.Precio) Total
FROM	Servicios S
INNER JOIN	TiposServiciosConductores TSC
ON		S.TipoServicioConductor_ID = TSC.TipoServicioConductor_ID
WHERE	S.Sesion_ID = @Sesion_ID
AND		S.TipoServicioConductor_ID IS NOT NULL
AND		S.EstatusServicio_ID <> 4
GROUP BY	TSC.Nombre, S.Precio";
...
GROUP BY	TSC.Nombre, S.Precio";
    ...
    public class Vista_FlujoMonetarioDeServiciosDeSesion_Consolidado
    {
        ...
        {
            string sqlstr = "SELECT	M.Nombre Moneda, COUNT(S.Servicio_ID) Cantidad, SUM(S.Precio) Total \r\n" +
            "FROM	Servicios S \r\n" +
            "INNER JOIN	Monedas M \r\n" +
            "ON		M.Moneda_ID = S.Moneda_ID \r\n" +
            "WHERE	S.Sesion_ID = @Sesion_ID \r\n" +
            "AND		S.EstatusServicio_ID <> 4 \r\n" +
            "GROUP BY	M.Nombre";
...
GROUP BY	M.Nombre";
    ...
    public class Vista_DatosDeSesion
    {
        ...
        {
            string sqlstr = "SELECT	S.Sesion_ID, E.Nombre Empresa, \r\n" +
            "		EST.Nombre Estacion, \r\n" +
            "		C.Nombre Caja, \r\n" +
            "		U.Nombre + ' ' + U.Apellidos Vendedor, \r\n" +
            "		S.FechaInicial, \r\n" +
            "		ISNULL(S.FechaFinal, GETDATE()) FechaFinal \r\n" +
            "FROM	Sesiones S \r\n" +
            "INNER JOIN	Empresas E \r\n" +
            "ON		S.Empresa_ID = E.Empresa_ID \r\n" +
            "INNER JOIN	Estaciones EST \r\n" +
            "ON		EST.Estacion_ID = S.Estacion_ID \r\n" +
            "INNER JOIN	Cajas C \r\n" +
            "ON		S.Caja_ID = C.Caja_ID \r\n" +
            "INNER JOIN	Usuarios U \r\n" +
        ...
        public static DataTable GetDataTable(System.Int32 sesion_id)
        {
            string sqlstr = "SELECT	S.Sesion_ID, 'CASCO' Empresa, \r\n" +
            "		EST.Nombre Estacion, \r\n" +
            "		C.Nombre Caja, \r\n" +
            "		U.Nombre + ' ' + U.Apellidos Vendedor, \r\n" +
            "		S.FechaInicial, \r\n" +
    ...
    public class Vista_ComisionistasServicios
    {
        ...
        public static List<Vista_ComisionistasServicios> Get()
        {
            ...
            "FROM	Empresas E \r\n" +
            "INNER JOIN	ComisionesServicios CS \r\n" +
            "ON		E.Empresa_ID = CS.Comisionista_ID";
    ...
    public class ServiciosPendientesConductor
    {
		...
		S.Empresa_ID,
		S.Cuenta_ID,
		S.Precio, S.PagoConductor,
		CASE
		WHEN	S.Zona_ID IS NOT NULL
		THEN	Z.Nombre
		ELSE	TSC.Nombre
		END	Destino,
		S.Productividad,
		S.TipoServicioConductor_ID
FROM	Servicios S
LEFT JOIN	Zonas Z
ON		S.Zona_ID = Z.Zona_ID
LEFT JOIN	TiposServiciosConductores TSC
...
AND     S.Caja_ID <> 10
AND		S.FechaServicio BETWEEN @FechaInicial AND @FechaFinal
AND		S.PagoConductor IS NOT NULL
AND		S.FechaPago IS NULL
AND		S.EstatusServicio_ID = 2
UNION
SELECT	S.Servicio_ID, 
		S.Empresa_ID,
		S.Cuenta_ID,
...
AND		S.EstatusServicio_ID = 2";
    ...
    public class Vista_BoletosTicketPago
    {
        ...
        {
            string sqlstr = "SELECT	S.Servicio_ID, S.Ticket_ID, \r\n" +
            "		S.Unidad_ID, S.Conductor_ID, \r\n" +
            "		S.Empresa_ID, S.Cuenta_ID, \r\n" +
            "		S.Precio, S.PagoConductor \r\n" +
            "FROM	Servicios S \r\n" +
            "INNER JOIN	Tickets T \r\n" +
            "ON		S.Ticket_ID = T.Ticket_ID \r\n" +
    ...
    public class Vista_BitacoraUnidades
    {
		...
		C.Conductor_ID,
		UPPER(C.Apellidos + ' ' + C.Nombre) Conductor,
		E.Nombre EstatusOperativo,
		U.UltimaActualizacion Fecha,
		U.Estacion_ID
FROM	Unidades U
INNER JOIN	Conductores C
ON		U.ConductorOperativo_ID = C.Conductor_ID
INNER JOIN	EstatusOperativosUnidades E
ON		U.EstatusOperativoUnidad_ID = E.EstatusOperativoUnidad_ID
WHERE	U.Estacion_ID = @Estacion_ID
AND		U.EstatusOperativoUnidad_ID = 2
ORDER BY	U.UltimaActualizacion";
